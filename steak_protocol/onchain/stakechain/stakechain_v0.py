...
def check_valid_stake_holder(
    ...
    stake_output: TxOut,
    own_prev_state: StakeChainV0State,
    tx_info: TxInfo,
):
    """
...
def new_chain_state(
    own_next_state: StakeChainV0State,
    own_prev_state: StakeChainV0State,
    tx_info: TxInfo,
    elected_slot_leader: int,
) -> CoreChainState:
...
def check_correct_new_registered_state(
    ...
    new_desired_state = StakeChainV0State(
...
def check_correct_new_updated_mined_state(
    ...
    desired_new_state = StakeChainV0State(
        params,
        new_desired_holder_state,
...
def number_stake_holders_spent(
    own_prev_state: StakeChainV0State, tx_info: TxInfo
) -> int:
...
def check_no_stake_holder_spent(own_prev_state: StakeChainV0State, tx_info: TxInfo):
    assert (
        number_stake_holders_spent(own_prev_state, tx_info) == 0
    ), "Tried to unlock tokens from registered holder"


def check_one_stake_holder_spent(own_prev_state: StakeChainV0State, tx_info: TxInfo):
    assert (
        number_stake_holders_spent(own_prev_state, tx_info) == 1
    ), "Tried to unlock more tokens from registered holder"


def check_burn_one_auth_nft(own_prev_state: StakeChainV0State, tx_info: TxInfo):
    stakeholder_auth_nft = own_prev_state.params.stakeholder_auth_nft
...
def check_mint_one_auth_nft(own_prev_state: StakeChainV0State, tx_info: TxInfo):
    stakeholder_auth_nft = own_prev_state.params.stakeholder_auth_nft
...
def check_no_auth_nft_mint(own_prev_state: StakeChainV0State, tx_info: TxInfo):
    stakeholder_auth_nft = own_prev_state.params.stakeholder_auth_nft
    assert not stakeholder_auth_nft.policy_id in tx_info.mint.keys(), "Auth NFT minted"


def validator(
    state: StakeChainV0State, redeemer: StateRedeemer, context: ScriptContext
) -> None:
    ...
    if isinstance(redeemer, UpgradeProtocol):
        ...
        own_next_state: StakeChainV0State = resolve_datum_unsafe(
